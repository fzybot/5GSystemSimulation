...
{
	float dy = (y - cell_->getCoordinateY());
	float dx = (x - cell_->getCoordinateX());
	float angle = 0;
...
//	int x= cell_.getPhyEntity()->getAntennaArray()->getBeamContainer()[0][0];
    int x= cell_->getCoordinateX();
            y= cell_->getCoordinateY();
    ...
    for(int dirX=0; dirX<lonc; ++dirX){
        for(int dirY=0; dirY<latc; ++dirY){
            ...
            for(int k=1; k<=L; ++k){
				...
				float gain = cell_.getPhyEntity()->getAntennaArray()->getBeamContainer()[0][0].calculateAntGain(azimuth, 0, 0);
                if(kIn == 0)
                {
						...
						float cell_.getPhyEntity()->getAntennaArray()->getBeamContainer()[0][0]
